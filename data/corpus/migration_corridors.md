# Migration Corridors and Early Warning Indicators
...
networks that pull later waves toward the same destinations), and
language or cultural proximity (destinations that share a language,
religion, or cultural background with the origin region lower the
non-economic cost of relocating). Corridors tend to be persistent over
time: once established, a corridor between a given origin region and a
given receiving city will typically keep channeling most subsequent
migration flows from that origin, even when other, objectively closer
cities exist.

## Early warning indicators for an emerging corridor

Analysts can use several early warning indicators to detect an emerging
...
Together, these three signals allow a corridor to be flagged as emerging
months before it would be visible in slower-moving official migration
statistics, which is the central value proposition of an early warning
approach over a purely retrospective one.
