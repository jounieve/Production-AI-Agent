...
training data alone and has no access to live city indicators. In the other hand, a search
...
neither tool provides alone: 

(1) it queries a structured city-indicator
database in real time via MCP tools, 

(2) it retrieves and reranks
passages from a domain corpus to ground every claim, and 
...
city-profile numbers from the MCP tool, this is an output that would otherwise
require manually reading 4–5 documents and cross-referencing a
spreadsheet (therefore a lot more efficient).
...
At a high level, the pipeline executes the following stages:
...
4. **Self-Consistency synthesis** (`reasoning.py`) — The synthesis prompt is executed three times (k=3) independently using few-shot CoT; candidates are clustered by
...
We chose Reciprocal Rank Fusion instead of combining raw BM25 and cosine scores because the two scoring systems operate on different scales. While score normalisation is possible, it becomes sensitive to corpus size and score distribution. Since RRF only relies on ranking positions, it remained stable as we added or modified documents, making it a better fit for our relatively small but evolving corpus.

A second design decision worth naming: the LLM call is not hard-coded to one provider. agent.py and reasoning.py both read a single LLM_PROVIDER environment variable (openai or ollama) and build their OpenAI-SDK client against the corresponding endpoint - both providers speak the same chat.completions API, so no per-provider branching exists in the tool-calling loop itself. This let us develop and smoke-test the full tool-calling loop against a local Ollama model (zero marginal cost, no API key) before running the graded evaluation against OpenAI, without touching the pipeline code at all.

---

## 3. Evaluation

We used RAGAS using `eval/ragas_eval.py` comparing `basic_retrieval()`
...
context list; the benign chunks from the same query remained. As a result, the synthesis stage never saw the injected instruction.
...
the uncertainty of the output visible on every response. Although not strictly required for a limited-risk system, we also implemented the L1 filters and token-budget controls to improve robustness.
